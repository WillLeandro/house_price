"""
extratores/extrator_terrasanta.py

Extrator da Terra Santa Empreendimentos Imobiliarios (terrasantaimoveis.com.br).
Segue o contrato: expoe extrair() -> list[dict]

APOSTA DE ENGENHARIA (26/07/2026): a URL de listagem que recebemos
(.../imobiliaria/locacao/apartamento-kit-net-kitnet/imoveis/20936) tem a
MESMA ESTRUTURA das imobiliarias que ja confirmamos rodar em Code49
(Franjovi, BNZ, Almeida Silva, Giuliano, Solara). Alem disso, o robots.txt
da Terra Santa segue o mesmo padrao "Allow: / geral, bloqueio soh de bots
nomeados" que ja vimos nessas outras. Por isso, reaproveitamos a mesma
logica de parsing sem alteracoes -- SE FALHAR ao rodar de verdade, avisar
que a plataforma pode ser diferente do que apostamos.

Plataforma: Code49 (aposta, a confirmar rodando de verdade).
"""
...
BASE_URL = "https://www.terrasantaimoveis.com.br"
LISTAGEM_URL_BASE = "https://www.terrasantaimoveis.com.br/imobiliaria/locacao/apartamento-kit-net-kitnet/imoveis/20936"
...
def _extrair_pagina(url, sessao):
    ...
    if not articles:
        print(
            "AVISO: nenhum card 'article.c49-property-card' encontrado. "
            "A aposta de que a Terra Santa usa Code49 pode estar errada. "
            "Salve o HTML (Ctrl+U) e me envie um trecho pra eu ajustar."
        )
...
def extrair(max_paginas=20):
    ...
    print(f"Total coletado na Terra Santa: {len(todos_imoveis)} imoveis")
